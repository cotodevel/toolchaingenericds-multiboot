...
bool ReloadNDSBinaryFromContext(char * filename) __attribute__ ((optnone)) {
	...
	//copy loader code (arm7ibootldr.bin) to ARM7's EWRAM portion while preventing Cache issues
	coherent_user_range_by_size((uint32)&arm7ibootldr[0], (int)arm7ibootldr_size);					
	memcpy ((void *)NDS_LOADER_IPC_BOOTSTUBARM7_CACHED, (u32*)&arm7ibootldr[0], arm7ibootldr_size); 	//memcpy ( void * destination, const void * source, size_t num );	//memset(void *str, int c, size_t n)
	
	int headerSize = sizeof(struct sDSCARTHEADER);
	u8 * NDSHeader = (u8 *)TGDSARM9Malloc(headerSize*sizeof(u8));
	memcpy(NDSHeader, (u8*)&stage2_9i[0], headerSize);
	...
	int fileSize = stage2_9i_size;
	NDS_LOADER_IPC_CTX_UNCACHED_NTR->fileSize = fileSize;
	
	nocashMessage("tgds_multiboot_payload_twl.bin:ReloadNDSBinaryFromContext1():");
	...
	memcpy(outBuf7, (u8*)&stage2_9i[arm7BootCodeOffsetInFile], arm7BootCodeSize);
	
	coherent_user_range_by_size((u32)&stage2_9i[arm9BootCodeOffsetInFile], (int)arm9BootCodeSize);	//prevent cache problems
	memcpy(outBuf9, (u8*)&stage2_9i[arm9BootCodeOffsetInFile], arm9BootCodeSize);
	...
	sprintf(msgDebug,"%s%x","tgds_multiboot_payload_twl.bin:ReloadNDSBinaryFromContext() ARM9 Boot Addr: ", (u32)outBuf9);
